...
module.exports = class DataDocument {
   ...
   * @param {string} client A DashJS client, with options set for the locator of the docs to be retrieved
   ...
   */
  static async find(client, locator, query) {
    debug(
    ...
    try {
      ...
      //if (numDocsFound > 0) {
      //REM - just return empty array of no docs found (or throw error if caught)
       ...
       // REM : just return array of docs (or error if caught)
       return arrDocs;
      /*
      } else {
        debug(`No docs found`);
